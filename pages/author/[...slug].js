...
export default function SingleNews() {
    ...
    useEffect(() => {
      axios.get(`${process.env.NEXT_AVETTI_CMS}/writers?name=${router.query.slug}`)
        .then(res => {
                ...
                <div className="card-left">
                  ...
                  <div className="card-left">
                    {author && author.articles && (
                      <>
                        <img src={author.picture.url} className="profile"></img>
                        <h1 className="titleText">
                          <span style={{ fontWeight: "300" }}>
                            About {author.name}
                          </span>
                        </h1>
                        <p className="author-info">{author.description}</p>
                        <p className="author-info">{`So far ${author.name} has created ${author.articles.length} articles.`}</p>
                        <hr className="uk-divider-small" style={{marginTop:"20px"}}/>
                      </>
                    )}
                    {author.articles?.map(el => (
                      <>
