...
import classes from "./Internationalization.module.css";
import Dropdown from "../AC-UI-Elements/Dropdown/Dropdown";
import { useWeglot } from "../../store-config";
...
export default function Internationalization({ locale }) {
  ...
  useEffect(() => {
    if (useWeglot && isBrowser && window.Weglot && weglotInitializedOnceState) {
    ...
    return () => {
      if (useWeglot && callbackRef.current)
  ...
  console.log("availableLanguages", availableLanguages, langNames);

  useEffect(() => {
    if (availableLanguages?.length === 0 && useWeglot === false) {
      setAvailableLanguages(
        process.env.NEXT_PUBLIC_AVAILABLE_LANGUAGES?.split(",").map(lang => {
          if (lang === "en") {
            return { [lang]: "English" };
          } else if (lang === "es") {
            return { [lang]: "Spanish" };
    ...
    <>
      <div
        ...
        className={`${classes.container}${
          weglotInitializedOnceState || useWeglot === false
            ? " " + classes.initialized
            : ""
        }`}
      >
